-- Revert the Nauru guests' terrace review from homepage testimonials.
...
SET published_version_id = (
        ...
        WHERE v.page_id = cms_pages.id
          AND v.id <> 'dddddddd-dddd-dddd-dddd-dddddddddd24'::uuid
        ORDER BY v.version_number DESC
        LIMIT 1
    ),
    updated_at = NOW()
WHERE id = 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaa1'
  AND published_version_id = 'dddddddd-dddd-dddd-dddd-dddddddddd24'::uuid;

DELETE FROM cms_versions
WHERE id = 'dddddddd-dddd-dddd-dddd-dddddddddd24'::uuid;
