...
func RegisterAdminRoutes(router fiber.Router, handler *Handler, authenticate fiber.Handler) {
	...
	admin.Post("/tables", handler.CreateTable)
	admin.Patch("/tables/:id", handler.UpdateTable)
	admin.Delete("/tables/:id", handler.DeleteTable)
	admin.Get("/holidays", handler.ListClosedDays)
	admin.Post("/holidays", handler.CreateClosedDay)
	admin.Patch("/holidays/:id", handler.UpdateClosedDay)
	admin.Delete("/holidays/:id", handler.DeleteClosedDay)
